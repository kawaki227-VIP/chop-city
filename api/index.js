...
app.post('/api/products', upload.array('images',5), async (req,res)=>{
  try{
    ...
    for(const f of (req.files||[])){
      const ext=(f.originalname.match(/\.[a-z0-9]+$/i)||['.jpg'])[0];
      const b=await put(`chop-city/images/${p.id}-${id()}${ext}`,f.buffer,blobOpts({contentType:f.mimetype}));
      p.images.push(b.url);
    }
  ...
  }catch(e){console.error(e);res.status(500).json({error:'Erreur lors de la publication'});}
});
